...
@userge.on_cmd("settings", about={"header": "Just For Fun"}, allow_channels=False)
...
async def send_alive_message(message: Message) -> None:
    ...
    if client.is_bot:
        ...
        caption += (
            f"\n⚠️  <a href={Config.UPSTREAM_REPO}><b>LINK</b></a>"
            "    <code>|</code>    "
            "  <a href='https://t.me/xLALO'><b>HELP</b></a>"
        )
...
class Bot_Alive:
    ...
    def alive_info() -> str:
        alive_info_ = f"""
<a href="https://t.me/joinchat/EodhGjP6GjkyYzgx"><b>   SCUMZ   </a>CLICK TO JOIN SCUMZ</b>
    ...
    def alive_buttons() -> InlineKeyboardMarkup:
        buttons = [
            [
                ...
                InlineKeyboardButton(text="⚠️LINK⚠️", url=Config.UPSTREAM_REPO),
            ]
        ]
